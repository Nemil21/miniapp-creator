...
export const ChatInterface = forwardRef<ChatInterfaceRef, ChatInterfaceProps>(
  function ChatInterface(
    { currentProject, onProjectGenerated, onGeneratingChange, activeAgent },
    ref
  ) {
    ...
    const shouldBlockChat =
      !credsOff && !!(activeAgent && walletAddress && !hasEnoughCredits);
      ...
      const loadChatMessages = async () => {
        ...
        if (currentProject?.projectId && sessionToken) {
          ...
          if (currentPhase !== "editing") {
            logger.log(
              "🔍 Setting phase to editing for existing project:",
          ...
          try {
            ...
            const response = await fetch(
              `/api/chat?projectId=${currentProject.projectId}`,
              {
    ...
    useEffect(() => {
      logger.log(
        "🔄 onGeneratingChange called with isGenerating:",
        isGenerating
      );
    ...
    const handleSendMessage = async (userMessage: string) => {
      ...
      if (shouldBlockChat) {
        toast.error(
          "Insufficient credits. Please top up your balance to continue chatting."
        );
        return;
      }
      ...
      try {
          ...
          try {
            ...
            if (updateResponse.status === 202) {
              ...
              setChat((prev) => {
                ...
                if (
                  newChat.length > 0 &&
                  newChat[newChat.length - 1].role === "ai"
                ) {
                  newChat[
                    newChat.length - 1
                  ].content = `Processing your changes... This may take 2-5 minutes. Job ID: ${jobData.jobId.substring(
                    0,
                    8
                  )}...`;
                }
              ...
              if (currentProject) {
                const updatedProject: GeneratedProject = {
                  ...
                  url:
                    result.previewUrl || result.vercelUrl || currentProject.url,
                  lastUpdated: Date.now(), // Add timestamp to force iframe refresh
                };
                logger.log(
                  "🔄 Updating project with timestamp:",
                  updatedProject.lastUpdated
                );
              ...
              if (currentProject?.projectId) {
                try {
                  await fetch(
                    `/api/projects/${currentProject.projectId}/chat`,
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
              ...
              logger.log(
                "✅ Changes applied successfully (sync mode):",
                updateData.changed
              );
              ...
              const successContent = `Changes applied successfully! I've updated ${
                updateData.changed?.length || 0
              } files. The preview should reflect your changes shortly.`;
              ...
              setChat((prev) => {
                ...
                if (
                  ...
                  newChat[newChat.length - 1].content = successContent;
                  newChat[newChat.length - 1].changedFiles =
                    updateData.changed || [];
                }
                return newChat;
              });
            ...
            const errorContent =
              "❌ Sorry, I encountered an error while applying the changes. Please try again.";
            ...
            if (currentProject?.projectId) {
              try {
                ...
                logger.warn(
                  "Failed to save AI error message to database:",
                  error
                );
              }
            }
        ...
        if (currentPhase === "requirements" && !isGenerating) {
          ...
          const isConfirmedByText =
            aiResponseLower.includes("proceed to build") ||
          ...
          if (isConfirmedByText || isConfirmedByAPI) {
            logger.log(
              "✅ Project confirmation detected! Transitioning to building phase...",
              {
                isConfirmedByText,
                isConfirmedByAPI,
                isGenerating,
            ...
            logger.log(
              "🚀 Triggering project generation with AI analysis:",
            ...
            if (generationTimeoutRef.current) {
              clearTimeout(generationTimeoutRef.current);
              logger.log(
                "🧹 Cleared existing generation timeout to prevent duplicates"
              );
            }
          ...
          if (phase === "editing") {
            logger.log(
              "📝 In editing phase - generation not allowed, only file modifications"
            );
          }
        }
      } catch (err) {
    ...
    const pollJobStatus = async (jobId: string): Promise<GeneratedProject> => {
      ...
      return new Promise((resolve, reject) => {
        const pollInterval = setInterval(async () => {
          ...
          try {
            logger.log(
              `🔄 Polling job ${jobId} (attempt ${attempt}/${maxAttempts})...`
            );
            ...
            if (!response.ok) {
              ...
              reject(
                new Error(`Failed to fetch job status: ${response.status}`)
              );
              return;
            }
            ...
            if (job.status === "completed") {
              ...
              const errorMessage =
                job.result?.deploymentError || job.error || "Job failed";
              reject(new Error(errorMessage));
            } else if (attempt >= maxAttempts) {
              clearInterval(pollInterval);
              reject(
                new Error("Job polling timeout - generation is taking too long")
              );
            }
    ...
    const handleGenerateProject = async (generationPrompt: string) => {
      ...
      try {
        logger.log(
          ...
          generationPrompt.substring(0, 200) + "..."
        );

        // Check if async processing is enabled
        const useAsyncProcessing =
          window.localStorage.getItem("minidev_use_async_processing") ===
            "true" || process.env.NEXT_PUBLIC_USE_ASYNC_PROCESSING === "true";

        // TEST MODE: Add this header to enable quick 30-second return for debugging
        const testQuickReturn =
          window.localStorage.getItem("minidev_test_quick_return") === "true";
        if (testQuickReturn) {
        ...
        if (response.status === 202 && useAsyncProcessing) {
          ...
          setChat((prev) => [
            ...
            {
              role: "ai",
              content: `⏳ Your miniapp generation has started! This will take about ${
                jobData.estimatedTime || "5-10 minutes"
          ...
          logger.log(
            "📦 Project generated successfully via async processing:",
            {
        ...
        setChat((prev) => [
          ...
          {
            role: "ai",
            content: displayMessage.includes("**Deployment Failed**")
              ? displayMessage
              : `❌ ${displayMessage}`,
            phase: "building",
            timestamp: Date.now(),
    ...
    useEffect(() => {
      ...
      window.addEventListener(
        "templateSelect",
        handleTemplateSelect as EventListener
      );
      return () => {
    ...
    return (
      <div className="flex-1 w-full flex flex-col bg-[#0000000A] max-h-full">
        ...
        <div
          ...
          className="flex-1 overflow-y-auto px-[20px] pt-4"
        >
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-xs font-semibold text-green-600">
                            {msg.changedFiles.length} file
                            {msg.changedFiles.length !== 1 ? "s" : ""} updated
                          </span>
                        </div>
                        <div className="text-xs text-gray-600 space-y-1">
                          ...
                          {msg.changedFiles.length > 3 && (
                            <div className="text-xs text-gray-400 ml-4">
                              +{msg.changedFiles.length - 3} more file
                              {msg.changedFiles.length - 3 !== 1 ? "s" : ""}
                            </div>
                          )}
                        </div>
                      </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-red-700">
                    <p className="font-normal text-xs">
                      Insufficient credits. Please top up to continue chatting
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-full px-4 py-2.5 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-yellow-700">
                    <p className="font-normal text-xs">
                      This is a beta version—stick to simple ideas, as complex
                      prompts may break or behave unexpectedly
                    </p>
                  </div>
                </div>
              </div>
