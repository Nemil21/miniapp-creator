...
export default function TopUpDialog({
  ...
  const [escrowContract, setEscrowContract] = useState<EscrowContract | null>(
    null
  );
  ...
  const handleTopUp = async (option: TopUpOption) => {
    ...
    try {
      ...
      const toastSending = toast.loading("Sending transaction...", {
        id: txToast,
      });

      const agentId = keccak256(
        toBytes(escrowContract.depositFunction.agentIdParam)
      );
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-black-60">
                          {option.amountInEth
                            ? parseFloat(option.amountInEth).toFixed(5)
                            : 0}{" "}
                          ETH
                        </span>
                      </div>
